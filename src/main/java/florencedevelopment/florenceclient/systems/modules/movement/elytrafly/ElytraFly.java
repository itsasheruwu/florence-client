...
package florencedevelopment.florenceclient.systems.modules.movement.elytrafly;
...
import florencedevelopment.florenceclient.mixin.AbstractBlockAccessor;
...
import florencedevelopment.florenceclient.systems.modules.movement.elytrafly.modes.Packet;
import florencedevelopment.florenceclient.systems.modules.movement.elytrafly.modes.Pitch40;
import florencedevelopment.florenceclient.systems.modules.movement.elytrafly.modes.Vanilla;
import florencedevelopment.florenceclient.systems.modules.player.ChestSwap;
import florencedevelopment.florenceclient.systems.modules.player.Rotation;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isGliding()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                    if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).florence$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3d) event.movement).florence$set(0, currentMode.velY, 0);
                }
            } else if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).florence$set(currentMode.velX, currentMode.velY, currentMode.velZ);
        ...
        if (autoHover.get() && mc.player.input.playerInput.sneak() && !Modules.get().get(Freecam.class).isActive() && mc.player.isGliding() && flightMode.get() != ElytraFlightModes.Bounce) {
            ...
            final boolean underCollidable = ((AbstractBlockAccessor) under).florence$isCollidable() || !underState.getFluidState().isEmpty();
            final boolean under2Collidable = ((AbstractBlockAccessor) under2).florence$isCollidable() || !under2State.getFluidState().isEmpty();

            if (!underCollidable && under2Collidable) {
                ((IVec3d)event.movement).florence$set(event.movement.x, -0.1f, event.movement.z);
            ...
            if (underCollidable) {
                ((IVec3d)event.movement).florence$set(event.movement.x, -0.03f, event.movement.z);
                ...
                if (mc.player.getEntityPos().y <= mc.player.getBlockPos().down().getY() + 1.34f) {
                    ((IVec3d)event.movement).florence$set(event.movement.x, 0, event.movement.z);
    ...
    protected void enableGroundListener() {
        FlorenceClient.EVENT_BUS.subscribe(staticGroundListener);
    }

    protected void disableGroundListener() {
        FlorenceClient.EVENT_BUS.unsubscribe(staticGroundListener);
    }
    ...
    protected void enableInstaDropListener() {
        FlorenceClient.EVENT_BUS.subscribe(staticInstadropListener);
    }

    protected void disableInstaDropListener() {
        FlorenceClient.EVENT_BUS.unsubscribe(staticInstadropListener);
    }
