...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(FlorenceRenderPipelines.POST_IMAGE);
        FlorenceClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(FlorenceClient.identifier("textures/chams." + fileFormat));
            ...
            FlorenceClient.LOG.error("Error loading the chams shader", e);
        }
    }
