...
public class EntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        for (Direction direction : DirectionAccessor.florence$getHorizontal()) {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) mc.world).florence$getEntityLookup();
        ...
        if (entityLookup instanceof SimpleEntityLookup<Entity> simpleEntityLookup) {
            SectionedEntityCache<Entity> cache = ((SimpleEntityLookupAccessor) simpleEntityLookup).florence$getCache();
            LongSortedSet trackedPositions = ((SectionedEntityCacheAccessor) cache).florence$getTrackedPositions();
            Long2ObjectMap<EntityTrackingSection<Entity>> trackingSections = ((SectionedEntityCacheAccessor) cache).florence$getTrackingSections();
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((EntityTrackingSectionAccessor) entityTrackingSection).<Entity>florence$getCollection()) {
