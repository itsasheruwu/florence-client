...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(FlorenceClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(FlorenceClient.MOD_ID, file.getName());
            ...
            FlorenceClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    FlorenceClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    FlorenceClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            FlorenceClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
