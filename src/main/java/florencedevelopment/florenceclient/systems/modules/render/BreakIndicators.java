...
import florencedevelopment.florenceclient.mixin.WorldRendererAccessor;
...
import florencedevelopment.florenceclient.systems.modules.world.PacketMine;
...
public class BreakIndicators extends Module {
    ...
    private void renderNormal(Render3DEvent event) {
        Map<Integer, BlockBreakingInfo> blocks = ((WorldRendererAccessor) mc.worldRenderer).florence$getBlockBreakingInfos();

        float ownBreakingStage = ((ClientPlayerInteractionManagerAccessor) mc.interactionManager).florence$getBreakingProgress();
        BlockPos ownBreakingPos = ((ClientPlayerInteractionManagerAccessor) mc.interactionManager).florence$getCurrentBreakingBlockPos();
