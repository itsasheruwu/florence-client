...
import florencedevelopment.florenceclient.events.florence.ActiveModulesChangedEvent;
...
import florencedevelopment.florenceclient.systems.modules.combat.*;
import florencedevelopment.florenceclient.systems.modules.misc.*;
import florencedevelopment.florenceclient.systems.modules.misc.swarm.Swarm;
...
import florencedevelopment.florenceclient.systems.modules.player.*;
import florencedevelopment.florenceclient.systems.modules.render.*;
import florencedevelopment.florenceclient.systems.modules.render.blockesp.BlockESP;
import florencedevelopment.florenceclient.systems.modules.render.marker.Marker;
import florencedevelopment.florenceclient.systems.modules.world.*;
...
import florencedevelopment.florenceclient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                FlorenceClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    /***
     * @see florencedevelopment.florenceclient.commands.commands.BindCommand
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        FlorenceClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    FlorenceClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : getAll()) {
                if (module.isActive() && !module.runInMainMenu) {
                    FlorenceClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
