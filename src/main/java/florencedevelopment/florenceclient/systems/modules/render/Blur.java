...
import florencedevelopment.florenceclient.renderer.FixedUniformStorage;
...
public class Blur extends Module {
    ...
    public Blur() {
        ...
        FlorenceClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
