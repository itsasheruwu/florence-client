...
import florencedevelopment.florenceclient.mixin.CrossbowItemAccessor;
import florencedevelopment.florenceclient.mixin.ProjectileInGroundAccessor;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double angleOffset, boolean accurate, float tickDelta) {
        ...
        switch (item) {
            ...
            case CrossbowItem ignored -> {
                ...
                float speed = CrossbowItemAccessor.florence$getSpeed(projectilesComponent);
    ...
    public boolean set(Entity entity) {
        ...
        if (entity instanceof ProjectileInGroundAccessor ppe && ppe.florence$invokeIsInGround()) return false;
    ...
    public SimulationStep tick() {
        age++;
        ((IVec3d) prevPos3d).florence$set(pos);
    ...
    private SimulationStep getCollision() {
        ...
        if (blockCollision.getType() != HitResult.Type.MISS) {
            ((IVec3d) pos3d).florence$set(blockCollision.getPos());
        }
