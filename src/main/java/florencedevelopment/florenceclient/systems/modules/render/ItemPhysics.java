...
import florencedevelopment.florenceclient.mixin.ItemRenderStateAccessor;
import florencedevelopment.florenceclient.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemRenderStateAccessor) event.renderState.itemRenderState).florence$getLayerCount(); i++) {
            ItemRenderState.LayerRenderState layer = ((ItemRenderStateAccessor) event.renderState.itemRenderState).florence$getLayers()[i];
            ModelInfo info = getInfo(layer.getQuads());

            matrices.push();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).florence$getTransform());
