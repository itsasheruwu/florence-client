...
package florencedevelopment.florenceclient.utils;
...
import florencedevelopment.florenceclient.mixin.*;
import florencedevelopment.florenceclient.mixininterface.IMinecraftClient;
import florencedevelopment.florenceclient.settings.StatusEffectAmplifierMapSetting;
...
import florencedevelopment.florenceclient.utils.player.EChestMemory;
import florencedevelopment.florenceclient.utils.render.PeekScreen;
...
import florencedevelopment.florenceclient.utils.world.BlockEntityIterator;
...
public class Utils {
    ...
    public static void init() {
        FlorenceClient.EVENT_BUS.subscribe(Utils.class);
    }
    ...
    public static int getRenderDistance() {
        return Math.max(mc.options.getViewDistance().getValue(), ((ClientPlayNetworkHandlerAccessor) mc.getNetworkHandler()).florence$getChunkLoadDistance());
    }
    ...
    public static void unscaledProjection() {
        ...
        RenderUtils.projection.set(((ProjectionMatrix2Accessor) matrix).florence$callGetMatrix(width, height));
    ...
    public static void getItemsInContainerItem(ItemStack itemStack, ItemStack[] items) {
        ...
        if (components.contains(DataComponentTypes.CONTAINER)) {
            ...
            DefaultedList<ItemStack> stacks = container.florence$getStacks();
    ...
    public static boolean hasItems(ItemStack itemStack) {
        ...
        if (container != null && !container.florence$getStacks().isEmpty()) return true;
    ...
    public static String getWorldName() {
        ...
        if (mc.isInSingleplayer()) {
            ...
            File folder = ((MinecraftServerAccessor) mc.getServer()).florence$getSession().getWorldDirectory(mc.world.getRegistryKey()).toFile();
    ...
    public static byte[] readBytes(InputStream in) {
        try {
            ...
            FlorenceClient.LOG.error("Error reading from stream.", e);
            return new byte[0];
        } finally {
    ...
    public static void leftClick() {
        ...
        int attackCooldown = ((MinecraftClientAccessor) mc).florence$getAttackCooldown();
        if (attackCooldown == 10000) {
            ((MinecraftClientAccessor) mc).florence$setAttackCooldown(0);
        }

        mc.options.attackKey.setPressed(true);
        ((MinecraftClientAccessor) mc).florence$leftClick();
        mc.options.attackKey.setPressed(false);
    }

    public static void rightClick() {
        ((IMinecraftClient) mc).florence$rightClick();
    }
    ...
    public static boolean isLoading() {
        ResourceReloadLogger.ReloadState state = ((ResourceReloadLoggerAccessor) ((MinecraftClientAccessor) mc).florence$getResourceReloadLogger()).florence$getReloadState();
        return state == null || !((ReloadStateAccessor) state).florence$isFinished();
    }
